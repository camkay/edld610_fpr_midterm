...
# nest by school, academic year, school district, and grade level and keep only the first 100 observations
plot_data <- data_es %>%
  ...
  nest(-school, -grade_level, -academic_year, -district, -gap_es) %>%
  #nest(-school) %>%
  slice(1:100)
...
plot_data %>%
  ...
  ggplot(aes(grade_level, gap_es)) +
    ...
    theme(legend.position = "bottom",
          legend.title = NULL) +
    labs(caption = "2014-2015 school year, Ashland School District, Oregon",
         y = "Effect Size",
         x = "Grade") +
...
indiv_plot <- function(data, var) {
    p <- ggplot(data, aes(grade_level, gap_es)) +
             geom_col(aes(fill = gap_es)) +
             scale_fill_carto_c(palette = "Burg") +
             coord_flip() +
             theme_minimal() +
        ...
        labs(title = var,
             ...
             x = "Grade") +
    if(var == "school") {
        p <- p + labs(title = "Achievment Gap Estimates:",
                      subtitle = var)
    }
    return(p)
}

map(plot_data, ~indiv_plot(.x, school))
