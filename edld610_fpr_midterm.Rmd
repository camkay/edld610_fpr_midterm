...
# split the data by school, academic year, and grade level
data_split <- data %>%
  # split by school
  split(.$school) %>%
  # split by grade level
  map(~split(.x, .x$grade_level)) %>%
  # split by academic year
  map(~map(.x, ~split(.x, .x$academic_year)))

# calculate effect sizes for each school by academic year and grade level
gap_es <- data_split %>%
  map(~map(.x, ~map(.x, ~gap(.x, "white", "hispanic_latino"))))

# create a dataframe with school, grade, and academic year in it
data_es <- data %>%
  filter(level == 1) %>%
  select(school, grade_level, academic_year, district) %>%
  arrange(school)

# extract all effect sizes from gap_es and add to data_es
test <- unlist(gap_es)
na.omit(unlist(gap_es))

data_es$gap_es <- c(unlist(gap_es), rep(NA, times = (nrow(data_es) - length(unlist(gap_es)))))
