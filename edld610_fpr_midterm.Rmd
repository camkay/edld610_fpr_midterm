...
map_dbl(plot_data$data, ~mean(.x$gap))

test <- plot_data %>%
    #gather(school, grade_level, academic_year) %>%
    mutate(gap = map_dbl(plot_data$data, ~mean(.x$gap)))

test %>%
  ggplot(aes(grade_level, gap)) +
...
plots_by_month <- map(test, function(x) {
