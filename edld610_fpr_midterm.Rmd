...
### Example
gap(data[1:4, ], "white", "hispanic_latino") #should be -0.6091404
```
...
test <- split(data, data$academic_year, data$school, data$grade_level)

map_dbl()


by_school <- split(data, data$school)

by_school %>%
  map(~gap(.x, "white", "hispanic_latino"))

#now we need to figure out how to do the above for every grade level in each academic year
