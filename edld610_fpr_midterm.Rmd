...
plot_data <- data_es %>%
  nest(-school, -grade_level, -academic_year, -district) %>%
  slice(1:100)

str(plot_data$data)

map_dbl(plot_data$data, ~mean(.x$gap_es))

test <- plot_data %>%
    #gather(school, grade_level, academic_year) %>%
    mutate(gap = map_dbl(plot_data$data, ~mean(.x$gap_es)))
...
plots_by_month <- map(test, function(x) {
  ggplot(x, aes(grade_level, gap_es)) +
    geom_col(aes(fill = gap_es)) +
    scale_fill_viridis_c() +
    coord_flip()
})
